...
class UsersController < ApplicationController
  ...
  def show
    ...
    @rating = Rating.find_by_sql(["SELECT u.name, user_id, round((score)::numeric, 1) AS score, content, tutor_id, r.id FROM ratings r JOIN users u ON u.id = r.user_id
                                 WHERE tutor_id = ?
                                ORDER BY r.created_at ASC", params[:id]]).paginate(page: params[:rate], per_page: 5)
    @average = User.find_by_sql(["SELECT coalesce(round((AVG(score))::numeric,1), -1) AS average FROM users u LEFT JOIN ratings r ON u.id = r.tutor_id
